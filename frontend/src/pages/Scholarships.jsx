...
const Scholarships = () => {
    ...
    const onApplyClick = (scholarshipId) => {
        ...
        if (userProfile) {
            ...
            if (failures.length > 0) {
                setToast({
                    show: true,
                    message: `Not eligible: ${failures[0]}`,
                    type: 'error'
                });
                return;
            }
    ...
    const handleSwitchConfirm = async () => {
        try {
            ...
            setToast({
                show: true,
                message: "Application switched successfully!",
                type: 'success'
            });
            ...
            setToast({
                show: true,
                message: error.response?.data?.detail || "Failed to switch scholarship.",
                type: 'error'
            });
        } finally {
            setLoading(false);
    ...
    return (
        <>
            {/* Toast - Outside container to maintain fixed positioning */}
            {toast.show && (
                <Toast
