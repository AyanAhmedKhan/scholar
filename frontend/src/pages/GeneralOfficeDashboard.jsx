...
const GeneralOfficeDashboard = () => {
    ...
    // Status update modal
    const [statusModal, setStatusModal] = useState({ open: false, appId: null, status: null });
    const [remarks, setRemarks] = useState('');
    
    // Document verification modal
    const [docModal, setDocModal] = useState({ open: false, docId: null, isVerified: null });
    ...
    const fetchApplications = async () => {
        ...
        try {
            ...
            const studentIds = [...new Set(res.data.map(app => app.student_id))];
            const studentPromises = studentIds.map(id => 
                api.get(`/profile/me`).catch(() => null)
            );
            // Note: This would need a proper endpoint to get student by ID
            // For now, we'll use the student relationship from the application
        } catch (e) {
    ...
    const handleDocumentVerify = async () => {
        if (!docModal.docId || docModal.isVerified === null) return;
        
        if (!docModal.isVerified && !docRemarks.trim()) {
            showError('Remarks are required when rejecting a document');
            return;
        }
        
        setLoading(true);
        try {
            await api.put(`/admin/applications/documents/${docModal.docId}/verify`, {
                is_verified: docModal.isVerified,
                remarks: docRemarks || null
            });
            showSuccess(`Document ${docModal.isVerified ? 'verified' : 'rejected'} successfully`);
            setDocModal({ open: false, docId: null, isVerified: null });
            setDocRemarks('');
            fetchApplications();
        } catch (err) {
            showError(err.response?.data?.detail || 'Failed to verify document');
        } finally {
            setLoading(false);
        }
    };
                                                ...
                                                {filteredApplications.map((app) => (
                                                    <React.Fragment key={app.id}>
                                                        <tr className={`hover:bg-slate-50 cursor-pointer transition-colors ${expandedAppId === app.id ? 'bg-slate-50' : ''}`} onClick={() => toggleExpand(app.id)}>
                                                            ...
                                                            <td className="px-6 py-4">
                                                                <div className="font-medium text-slate-800">Student #{app.student_id}</div>
                                                                {app.student?.full_name && (
                                                                    <div className="text-xs text-slate-500">{app.student.full_name}</div>
                                                                )}
                                                            </td>
                                                            <td className="px-6 py-4">
                                                        ...
                                                        {expandedAppId === app.id && (
                                                            <tr className="bg-slate-50/50">
                                                                <td colSpan="7" className="px-6 py-6">
                                                                    ...
                                                                    className="text-primary-600 text-xs hover:underline font-medium mt-1 inline-block"
                                                                                                >
                                                                                                    View File
                                                                                                </a>
                                                                                                {doc.remarks && (
                                                                                            ...
                                                                                            <div className="flex gap-2">
                                                                                                {doc.is_verified ? (
                                                                                                    ...
                                                                                                    <>
                                                                                                        <button
                                                                                                            onClick={() => setDocModal({ open: true, docId: doc.id, isVerified: true })}
                                                                                                        ...
                                                                                                        <button
                                                                                                            onClick={() => setDocModal({ open: true, docId: doc.id, isVerified: false })}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
                        <h3 className="text-lg font-bold text-slate-800 mb-4">
                            {docModal.isVerified ? 'Verify Document' : 'Reject Document'}
                        </h3>
                        <div className="space-y-4">
                            {!docModal.isVerified && (
                                <div>
                        ...
                        <div className="flex gap-3 mt-6">
                            <button
                                ...
                                disabled={loading || (!docModal.isVerified && !docRemarks.trim())}
                            ...
                            >
                                {loading ? 'Processing...' : docModal.isVerified ? 'Verify' : 'Reject'}
                            </button>
                            <button
                                onClick={() => {
                                    setDocModal({ open: false, docId: null, isVerified: null });
