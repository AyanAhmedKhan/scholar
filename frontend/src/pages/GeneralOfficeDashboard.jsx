...
const GeneralOfficeDashboard = () => {
    ...
    const fetchNotices = async () => {
        try {
            const res = await api.get('/notices/'); // Fetch all notices (protected endpoint)
            setNotices(res.data);
        } catch (e) {
    ...
    const handleSaveNotice = async (e) => {
        ...
        try {
            if (noticeModal.mode === 'create') {
                ...
                await api.put(`/notices/${noticeModal.data.id}`, payload);
                showSuccess('Notice updated successfully');
            }
            setNoticeModal({ open: false, mode: 'create', data: null });
            fetchNotices();
        } catch (err) {
            showError(err.response?.data?.detail || 'Failed to save notice');
        } finally {
            setLoading(false);
        }
    };
            ...
            {activeTab === 'notices' && (
                <div className="space-y-6">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-bold text-slate-800">Notice Board Management</h2>
                        <button
                            onClick={() => setNoticeModal({ open: true, mode: 'create', data: null })}
                            className="bg-primary-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-primary-700 transition-colors flex items-center gap-2 shadow-sm"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {notices.map(notice => (
                            <div key={notice.id} className={`bg-white rounded-xl shadow-sm border p-5 transition-all hover:shadow-md ${notice.is_active ? 'border-l-4 border-l-primary-500 border-slate-200' : 'border-l-4 border-l-slate-300 border-slate-200 opacity-75'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    <h3 className="font-bold text-slate-800 text-lg line-clamp-1" title={notice.title}>{notice.title}</h3>
                                    <div className="flex gap-1 -mr-2 -mt-2">
                                        <button
                                            onClick={() => setNoticeModal({ open: true, mode: 'edit', data: notice })}
                                ...
                                <p className="text-slate-600 text-sm mb-4 line-clamp-3 h-[4.5em]">{notice.content || 'No content'}</p>
                                <div className="flex justify-between items-center text-xs text-slate-500 pt-4 border-t border-slate-100">
                                    <span>{new Date(notice.created_at).toLocaleDateString()}</span>
                                    <span className={`px-2 py-0.5 rounded-full font-medium ${notice.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg animate-scale-in">
                        <form onSubmit={handleSaveNotice} className="p-6">
                            <h3 className="text-xl font-bold text-slate-800 mb-6">{noticeModal.mode === 'create' ? 'Create New Notice' : 'Edit Notice'}</h3>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
                                        required
                                        placeholder="e.g., Important: Deadline Extension"
                                    />
                                </div>
                                <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        name="is_active"
                                        id="notice_is_active"
                                        defaultChecked={noticeModal.mode === 'create' ? true : noticeModal.data?.is_active}
                                        className="w-4 h-4 text-primary-600 rounded focus:ring-primary-500"
                                    />
                                    <label htmlFor="notice_is_active" className="text-sm font-medium text-slate-700">Make this notice active visible to students</label>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-3 mt-8">
                                <button
                                    ...
                                    className="px-4 py-2.5 text-slate-600 font-semibold hover:bg-slate-50 rounded-lg transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="px-6 py-2.5 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 shadow-lg shadow-primary-900/20 transition-all disabled:opacity-50"
                                >
                                    {loading ? 'Saving...' : (noticeModal.mode === 'create' ? 'Create Notice' : 'Save Changes')}
                                </button>
                            </div>
                        </form>
                    </div>
